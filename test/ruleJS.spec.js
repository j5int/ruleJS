...
describe('ruleJS', function () {
  ...
  it('should have `helper` object', function () {
    expect(rules.helper).to.be.defined;
    expect(rules.helper).to.be.an.instanceof(Object);
  });

  it('should have `formulas` object', function () {
    expect(rules.formulas).to.be.defined;
    expect(rules.formulas).to.be.an.instanceof(Object);
  });

  it('should have `parse()` method', function () {
    expect(rules.parse).to.be.defined;
    expect(rules.parse).to.be.an.instanceof(Object);
  });

  describe('utils', function () {
    it('toNum() -> should convert A => 0, Z => 25, AA => 26', function () {
      expect(rules.utils.toNum('A')).to.equal(0);
      expect(rules.utils.toNum('Z')).to.equal(25);

      expect(rules.utils.toNum('AA')).to.equal(26);
      expect(rules.utils.toNum('AB')).to.equal(27);
    ...
    it('toChar() -> should convert 0 => A, 25 => Z, 26 => AA', function () {
      expect(rules.utils.toChar(0)).to.equal('A');
      expect(rules.utils.toChar(25)).to.equal('Z');
      expect(rules.utils.toChar(26)).to.equal('AA');
      expect(rules.utils.toChar(27)).to.equal('AB');
    });

    it('cellCoords() -> should return coordinates for cell ex. A1 -> {row: 0, col:0}', function () {
      expect(rules.utils.cellCoords('A1')).to.eql({row: 0, col: 0});
      expect(rules.utils.cellCoords('Z1')).to.eql({row: 0, col: 25});
      expect(rules.utils.cellCoords('AA1')).to.eql({row: 0, col: 26});
      expect(rules.utils.cellCoords('AB1')).to.eql({row: 0, col: 27});
    });

    it('translateCellCoords() -> should translate cell coordinates to cell ex. {row:0, col:0} -> A1', function () {
      expect(rules.utils.translateCellCoords({row: 0, col: 0})).to.equal('A1');
      expect(rules.utils.translateCellCoords({row: 0, col: 25})).to.equal('Z1');
      expect(rules.utils.translateCellCoords({row: 0, col: 26})).to.equal('AA1');
      expect(rules.utils.translateCellCoords({row: 0, col: 27})).to.equal('AB1');
    });

    it('changeColIndex() -> change column index ex. A1 -> B1', function() {
      expect(rules.utils.changeColIndex('A1', 1)).to.equal('B1');
      expect(rules.utils.changeColIndex('A1', -1)).to.equal('A1');
      expect(rules.utils.changeColIndex('AA1', 1)).to.equal('AB1');

      expect(rules.utils.changeColIndex('Z1', 1)).to.equal('AA1');
      expect(rules.utils.changeColIndex('AA1', -1)).to.equal('Z1');

      expect(rules.utils.changeColIndex('A1', 2)).to.equal('C1');
      expect(rules.utils.changeColIndex('AA1', 2)).to.equal('AC1');

      expect(rules.utils.changeColIndex('C1', -2)).to.equal('A1');
      expect(rules.utils.changeColIndex('AC1', -2)).to.equal('AA1');
    });

    it('changeRowIndex() -> change row index ex. A1 -> A2', function () {
      expect(rules.utils.changeRowIndex('A1', 1)).to.equal('A2');
      expect(rules.utils.changeRowIndex('A1', -1)).to.equal('A1');
      expect(rules.utils.changeRowIndex('AA1', 1)).to.equal('AA2');

      expect(rules.utils.changeRowIndex('Z1', 1)).to.equal('Z2');
      expect(rules.utils.changeRowIndex('AA1', -1)).to.equal('AA1');

      expect(rules.utils.changeRowIndex('A1', 2)).to.equal('A3');
      expect(rules.utils.changeRowIndex('AA1', 2)).to.equal('AA3');

      expect(rules.utils.changeRowIndex('C1', -2)).to.equal('C1');
      expect(rules.utils.changeRowIndex('AC1', -2)).to.equal('AC1');
    });

    it('getCellAlphaNum() -> get alpha and number of cell ex. A1 -> {alpha: A, num:1}', function () {
      expect(rules.utils.getCellAlphaNum('A1')).to.eql({alpha: 'A', num: 1});
      expect(rules.utils.getCellAlphaNum('AA1')).to.eql({alpha: 'AA', num: 1});
    });
  });
});
...
describe('parse()', function () {
  ...
  describe('math', function () {
    it('operator: +', function () {
      parsed = rules.parse('1+2');
      expect(parsed.result).to.equal(3);

      parsed = rules.parse('SUM(1,2) + SUM(2,3)');
      expect(parsed.result).to.equal(8);
    });

    it('operator: -', function () {
      parsed = rules.parse('2-1');
      expect(parsed.result).to.equal(1);

      parsed = rules.parse('SUM(2,3) - SUM(1,2)');
      expect(parsed.result).to.equal(2);
    });
    ...
    it('operator: /', function () {
      ...
      expect(parsed.result).to.equal(null);
      expect(parsed.error).to.equal('#DIV/0!');
    });
  });
  ...
  it('ACCRINT', function () {
    ...
    expect(parsed.result).to.equal(350);
  });

  it('ACOS', function () {
    parsed = rules.parse('ROUND(ACOS(-1),5)');
    expect(parsed.result).to.equal(3.14159);
  });

  it('ACOSH', function () {
    parsed = rules.parse('ROUND(ACOSH(10),5)');
    expect(parsed.result).to.equal(2.99322);
  });

  it('ACOTH', function () {
    parsed = rules.parse('ROUND(ACOTH(6),5)');
    expect(parsed.result).to.equal(0.16824);
  });
  ...
  it('ARABIC', function () {
    ...
    expect(parsed.result).to.equal(1912);
  });

  it('ASIN', function () {
    parsed = rules.parse("ROUND(ASIN(-0.5),5)");
    expect(parsed.result).to.equal(-0.5236);
  });

  it('ASINH', function () {
    parsed = rules.parse("ROUND(ASINH(-2.5),5)");
    expect(parsed.result).to.equal(-1.64723);
  });

  it('ATAN', function () {
    parsed = rules.parse("ROUND(ATAN(1),5)");
    expect(parsed.result).to.equal(0.7854);
  });

  it('ATAN2', function () {
    parsed = rules.parse("ROUND(ATAN2(-1, -1),5)");
    expect(parsed.result).to.equal(-2.35619);
  });

  it('ATANH', function () {
    parsed = rules.parse("ROUND(ATANH(-0.1),5)");
    expect(parsed.result).to.equal(-0.10034);
  });

  it('AVEDEV', function () {
    parsed = rules.parse('AVEDEV([2,4], [8,16])');
    expect(parsed.result).to.equal(4.5);
  });

  it('AVERAGE', function () {
    parsed = rules.parse("AVERAGE([2,4], [8,16])");
    expect(parsed.result).to.equal(7.5);
  });
  ...
  it('AVERAGEIF', function () {
    ...
    expect(parsed.result).to.equal(3.5);
  });

  it('BASE', function () {
    parsed = rules.parse("BASE(15, 2, 10)");
    expect(parsed.result).to.equal("0000001111");
  });

  it('BESSELI', function () {
    parsed = rules.parse("ROUND(BESSELI(1, 2),5)");
    expect(parsed.result).to.equal(0.13575);
  });

  it('BESSELJ', function () {
    parsed = rules.parse("ROUND(BESSELJ(1, 2),5)");
    expect(parsed.result).to.equal(0.1149);
  });

  it('BESSELK', function () {
    parsed = rules.parse("ROUND(BESSELK(1, 2),5)");
    expect(parsed.result).to.equal(1.62484);
  });

  it('BESSELY', function () {
    parsed = rules.parse("ROUND(BESSELY(1, 2),5)");
    expect(parsed.result).to.equal(-1.65068);
  });

  it('BETADIST', function () {
    parsed = rules.parse("ROUND(BETADIST(2, 8, 10, true, 1, 3),5)");
    expect(parsed.result).to.equal(0.68547);
  });
  ...
  it('BITLSHIFT', function () {
    ...
    expect(parsed.result).to.equal(704643072);
  });

  it('BITOR', function () {
    parsed = rules.parse("BITOR(42, 24)");
    expect(parsed.result).to.equal(58);
  });

  it('BITRSHIFT', function () {
    parsed = rules.parse("BITRSHIFT(42, 2)");
    expect(parsed.result).to.equal(10);
  });

  it('BITXOR', function () {
    parsed = rules.parse("BITXOR(42, 24)");
    expect(parsed.result).to.equal(50);
  });

  it('CEILING', function () {
    parsed = rules.parse("CEILING(-5.5, 2, -1)");
    expect(parsed.result).to.equal(-6);
  });
  ...
  it('DAYS360', function () {
    ...
    expect(parsed.result).to.equal(360);
  });

  it('DB', function () {
    parsed = rules.parse("DB(1000000, 100000, 6, 1, 6)");
    expect(parsed.result).to.equal(159500);
  });

  it('DDB', function () {
    parsed = rules.parse("DDB(1000000, 100000, 6, 1, 1.5)");
    expect(parsed.result).to.equal(250000);
  });

  it('DEC2BIN', function () {
    parsed = rules.parse("DEC2BIN(42)");
    expect(parsed.result).to.equal("101010");
  });
  ...
  it('DECIMAL', function () {
    ...
    expect(parsed.result).to.equal(255);
  });

  it('DEGREES', function () {
    parsed = rules.parse("DEGREES(PI())");
    expect(parsed.result).to.equal(180);
  });
  ...
  it('DEVSQ', function () {
    ...
    expect(parsed.result).to.equal(115);
  });

  it('DOLLAR', function () {
    parsed = rules.parse("DOLLAR(-0.123, 4)");
    expect(parsed.result).to.equal("($0.1230)");
  });

  it('DOLLARDE', function () {
    parsed = rules.parse("DOLLARDE(1.1, 16)");
    expect(parsed.result).to.equal(1.625);
  });

  it('DOLLARFR', function () {
    parsed = rules.parse("DOLLARFR(1.625, 16)");
    expect(parsed.result).to.equal(1.1);
  });

  it('E', function () {
    parsed = rules.parse("ROUND(E(),5)");
    expect(parsed.result).to.equal(2.71828);
  });

  it('EDATE', function () {
    parsed = rules.parse("EDATE('1/15/11', -1)");
    expect(parsed.result.toString()).to.equal("Wed Dec 15 2010 00:00:00 GMT+0100 (CET)");
  });

  it('EFFECT', function () {
    parsed = rules.parse("ROUND(EFFECT(0.1, 4),5)");
    expect(parsed.result).to.equal(0.10381);
  });

  it('EOMONTH', function () {
    parsed = rules.parse("EOMONTH('1/1/11', -3)");
    expect(parsed.result.toString()).to.equal("Sun Oct 31 2010 00:00:00 GMT+0200 (CEST)");
  });

  it('ERF', function () {
    parsed = rules.parse("ROUND(ERF(1),5)");
    expect(parsed.result).to.equal(0.8427);
  });

  it('ERFC', function () {
    parsed = rules.parse("ROUND(ERFC(1),5)");
    expect(parsed.result).to.equal(0.1573);
  });
  ...
  it('EXPONDIST', function () {
    ...
    expect(parsed.result).to.equal(0.86466);
  });
  ...
  it('FDIST', function () {
    ...
    expect(parsed.result).to.equal(0.00122);
  });

  it('FINV', function () {
    parsed = rules.parse("ROUND(FINV(0.01, 6, 4),5)");
    expect(parsed.result).to.equal(15.20686);
  });

  it('FISHER', function () {
    parsed = rules.parse("ROUND(FISHER(0.75),5)");
    expect(parsed.result).to.equal(0.97296);
  });

  it('FISHERINV', function () {
    parsed = rules.parse("FISHERINV(0.9729550745276566)");
    expect(parsed.result).to.equal(0.75);
  });

  it('INT', function () {
    parsed = rules.parse("INT(-8.9)");
    expect(parsed.result).to.equal(-9);
  });
  ...
  it('LN', function () {
    ...
    expect(parsed.result).to.equal(4.45435);
  });
  ...
  it('LOG10', function () {
    ...
    expect(parsed.result).to.equal(5);
  });

  it('MAX', function () {
    parsed = rules.parse("MAX([0.1,0.2], [0.4,0.8], [true, false])");
    expect(parsed.result).to.equal(0.8);
  });
  ...
  it('MIN', function () {
    ...
    expect(parsed.result).to.equal(0.1);
  });

  it('MINA', function () {
    parsed = rules.parse("MINA([0.1,0.2], [0.4,0.8], [true, false])");
    expect(parsed.result).to.equal(0);
  });
  ...
  it('POWER', function () {
    ...
    expect(parsed.result).to.equal(25);
  });

  it('ROUND', function () {
    parsed = rules.parse("ROUND(626.3, 2)");
    expect(parsed.result).to.equal(626.3);

    parsed = rules.parse("ROUND(626.3, -2)");
    expect(parsed.result).to.equal(600);
  });

  it('ROUNDOWN', function () {
    parsed = rules.parse("ROUNDDOWN(-3.14159, 2)");
    expect(parsed.result).to.equal(-3.14);
  });

  it('ROUNDUP', function () {
    parsed = rules.parse("ROUNDUP(-3.14159, 2)");
    expect(parsed.result).to.equal(-3.15);
  });

  it('SIN', function () {
    parsed = rules.parse("ROUND(SIN(1),5)");
    expect(parsed.result).to.equal(0.84147);
  });

  it('SINH', function () {
    parsed = rules.parse("ROUND(SINH(1),5)");
    expect(parsed.result).to.equal(1.1752);
  });

  it('SPLIT', function () {
    parsed = rules.parse("SPLIT('A,B,C', ',')");
    expect(parsed.result.join()).to.equal('A,B,C');
  });
  ...
  it('SQRTPI', function () {
    ...
    expect(parsed.result).to.equal(2.50663);
  });
  ...
  it('SUMIF', function () {
    ...
    expect(parsed.result).to.equal(24);
  });

  it('SUMIFS', function () {
    parsed = rules.parse("SUMIFS([2,4,8,16], [1,2,3,4], '>=2', [1,2,4,8], '<=4')");
    expect(parsed.result).to.equal(12);
  });
  ...
  it('SUMX2PY2', function () {
    ...
    expect(parsed.result).to.equal(30);
  });
  ...
  it('TAN', function () {
    ...
    expect(parsed.result).to.equal(1.55741);
  });

  it('TANH', function () {
    parsed = rules.parse("ROUND(TANH(-2),5)");
    expect(parsed.result).to.equal(-0.96403);
  });
